...
def weekhour_func(x):
    ...
    if rv.shape[0] == 1:
        ...
        tot_min = (x.iloc[1] - x.iloc[0]).seconds/60
        # proportion in ends
        beg_sub = (3600 - (x.iloc[0] - beg).seconds)/60
        end_sub = (x.iloc[1] - end).seconds/60
...
def agg_weekhour(data,begin,end,group=None):
    ...
    if group:
        ...
        agg_df = agg_df.merge(wkdf,how='left',on='weekval')
        agg_df['weight'] = agg_df['weight'].fillna(0.0)
